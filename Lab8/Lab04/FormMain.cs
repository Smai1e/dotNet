...
using ClassLibrary;
using ClassLibrary.Serialization;
...
namespace Lab04
{
    public partial class FormMain : Form
    {
        ...
        {
            var bookId = (int)sender;
            for (int i = 0; i < listViewBooks.Items.Count; i++)
            {
                if (((Book)listViewBooks.Items[i].Tag).BookId == bookId)
                {
        ...
        private void saveXMLToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                LibrarySerializable.Save(saveFileDialogMain.FileName, SerializeType.XML);
            }
        }
        ...
        private void saveJSONToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialogMain.ShowDialog() == DialogResult.OK)
            {
                LibrarySerializable.Save(saveFileDialogMain.FileName, SerializeType.JSON);
            }
        }
        ...
        private void saveBinaryToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialogMain.ShowDialog() == DialogResult.OK)
            {
                LibrarySerializable.Save(saveFileDialogMain.FileName, SerializeType.Binary);
            }
        }

        private void loadXMLToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            openFileDialogMain.Filter = "XML-файлы|*.xml|Все файлы|*.*";
            if (openFileDialogMain.ShowDialog() == DialogResult.OK)
            {
                LibrarySerializable.Load(openFileDialogMain.FileName, SerializeType.XML);
        ...
        private void loadJSONToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialogMain.ShowDialog() == DialogResult.OK)
            {
                LibrarySerializable.Load(openFileDialogMain.FileName, SerializeType.JSON);
            }
        }

        private void loadBinaryToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            openFileDialogMain.Filter = "Двоичные файлы|*.bin|Все файлы|*.*";
            if (openFileDialogMain.ShowDialog() == DialogResult.OK)
            {
                LibrarySerializable.Load(openFileDialogMain.FileName, SerializeType.Binary);
            }
        }
    }
}
